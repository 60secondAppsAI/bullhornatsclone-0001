...
<template>
        ...
        <card>
          <template slot="header">
            <div class="row justify-content-between align-items-between mx-3">
              ...
              <div>
                  <base-button class="btn btn-primary" @click="modalCandidates = true">Add</base-button>
              </div>
              
              <modal :show.sync="modalCandidates">
                <template slot="header">
  ...
  <base-input label="Resume" type="text" placeholder="Enter Resume" v-model="candidateToAdd.resume"></base-input>
          ...
          <div v-if="isTableView">
            ...
            <a-table :columns="columns" :data-source="candidates" :row-key="record => record.CandidateId" :pagination="pagination"
          ...
          <div v-else>
            ...
            <CandidatePictureView :candidates="candidates" />
          </div>
...
import CandidatePictureView from './CandidatePictureView.vue';
...
export default {
  ...
  components: {
    ...
    Pagination,
    CandidatePictureView
  },

  data() {
    return {
      modalCandidates: false,
        isTableView: true,
      ...
      sortBy: 'candidateId',           // Column to sort by
  ...
  methods: {
    ...
    async renderCandidatesTable() {
      ...
      for (let i = 0; i < this.candidates.length; i++) {
        candidatesTableData.push({
          id: i,
          candidateId: this.candidates[i].candidateId,
          year: this.candidates[i].year,
          date: this.candidates[i].date,
          competitionId: this.candidates[i].competitionId,
          candidateId: this.candidates[i].candidateId,
        });
    ...
    async onTableChange(pagination, filters, sorter) {
	  ...
	  this.$emit('get-all-candidates',this.sortBy,this.sortOrder,this.pagination.current-1,this.pagination.pageSize);
    ...
    handleSearchQueryChanged() {
    	...
    	this.$root.$emit('searchQueryForCandidatesChanged', this.searchQuery);
	...
	async handleAddSubmitted() {
      ...
      this.candidateToAdd.created = currentDate;
